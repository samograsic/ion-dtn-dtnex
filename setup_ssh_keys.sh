...
copy_key_to_node() {
    ...
    log "Setting up SSH key for ${USERNAME}@${hostname}:${port}..."
    
    # Use expect to automate ssh-copy-id with password
    expect << EOF
set timeout 30
spawn ssh-copy-id -o StrictHostKeyChecking=no -p ${port} ${USERNAME}@${hostname}
expect {
    "password:" {
        send "${PASSWORD}\r"
        expect {
            "Number of key(s) added:" {
                puts "Key successfully added to ${hostname}:${port}"
                exit 0
            }
            "ERROR:" {
                puts "Failed to add key to ${hostname}:${port}"
                exit 1
            }
            timeout {
                puts "Timeout adding key to ${hostname}:${port}"
                exit 1
            }
        }
    }
    "All keys were skipped because they already exist on the remote system." {
        puts "Keys already exist on ${hostname}:${port}"
        exit 0
    }
    "Connection refused" {
        puts "Connection refused by ${hostname}:${port}"
        exit 1
    }
    "No route to host" {
        puts "Cannot reach ${hostname}:${port}"
        exit 1
    }
    timeout {
        puts "Connection timeout to ${hostname}:${port}"
        exit 1
    }
}
EOF
    ...
    if [ $exit_code -eq 0 ]; then
        success "SSH key setup completed for ${hostname}:${port}"
        return 0
    else
        error "Failed to setup SSH key for ${hostname}:${port}"
        return 1
    fi
}
...
test_ssh_connection() {
    ...
    log "Testing SSH connection to ${hostname}:${port}..."
    
    if ssh -o ConnectTimeout=10 -o BatchMode=yes -p "${port}" "${USERNAME}@${hostname}" "echo 'SSH test successful'" >/dev/null 2>&1; then
        success "SSH connection test passed for ${hostname}:${port}"
        return 0
    else
        error "SSH connection test failed for ${hostname}:${port}"
        return 1
    fi
}
